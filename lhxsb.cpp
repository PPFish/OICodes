#include <cstdio>
#include <cstring>
#include <algorithm>
#include <cstdlib>
#include <cmath>
using namespace std;

const int maxn = 300100;
const double pi = acos(-1);
const int inf = 1e9;

struct point
{
	int x, y;

	bool operator < (const point& rhs) const
	{
		return x < rhs.x;
	}

} mts[maxn];


point R[maxn], L[maxn];
int n, q;
double ans[maxn];
bool vis[maxn];
pair<int, int> query[maxn];

void input()
{
	int x, y;
	scanf("%d%d", &n, &q);
	for (int i = 1; i <= n; i++) {
		scanf("%d%d", &mts[i].x, &mts[i].y);
	}
	sort(mts + 1, mts + n + 1);
	for (int i = 1; i <= q; i++) {
		scanf("%d%d", &x, &y);
		query[i] = make_pair(x, y);
		L[i] = mts[1];
		R[i] = mts[n];
		if (!x) vis[y] = true;
	}
	for (int i = 1; i <= n; i++) {
		if (!vis[i]) {
			query[++q] = make_pair(0, i);
		}
	}
}

struct Q
{
	int idx, pos;
	bool operator < (const Q& rhs) const {
		return mts[idx] < mts[rhs.idx];
	}
} Qs[maxn];

point Ds[maxn], stack[maxn];
int top;

long long cross(point& a, point& b, point& c)
{
	long long x1 = a.x - c.x, y1 = a.y - c.y, x2 = b.x - c.x, y2 = b.y - c.y;
	return x1 * y2 - y1 * x2;
}

void pushin(point& p, int d)
{                            
	// d = 1, 从左向右 , 反之 -1
	while (top > 1 && cross(stack[top], p, stack[top - 1]) * d > 0)
		top --;
	stack[++top] = p;
}

int find(int ind, int d)
{
	//寻找凸壳最近的点 d = 1, 从左到右, 反之 -1
	int l = 1, r = top, mid = (l + r + 1) >> 1;
	point& p = mts[ind];
	stack[0].x = stack[1].x, stack[0].y = -inf;
	while (r > l) {
		if (cross(stack[mid], p ,stack[mid - 1]) * d > 0) {
			r = mid - 1;
		} else {
			l = mid;
		}
		mid = (l + r + 1) >> 1;
	}
	return mid;
}

int pos[maxn];

int cmp(const int& a, const int& b)
{
	return mts[a] < mts[b];
}

void solve(int l, int r)
{
	//前半部分的询问，后半部分的加点。
	int mid, tot_Q = 0, tot_D = 0, cnt;
	top = 0;
	mid = (l + r) >> 1;
	for (int i = l; i <= mid; i++) {
		if (query[i].first) {
			cnt = query[i].second;
			Qs[++tot_Q].idx = cnt;
			Qs[tot_Q].pos = i;
		}
	}
	for (int i = mid + 1; i <= r; i++) {
		if (!query[i].first) {
			cnt = query[i].second;
			Ds[++tot_D] = mts[cnt];
		}
	}

	sort(Qs + 1, Qs + tot_Q + 1);
	sort(Ds + 1, Ds + tot_D + 1);

	int k;
	for (int i = 1, k = 1; i <= tot_Q; i++) {
		while (k <= tot_D && Ds[k].x < mts[Qs[i].idx].x) {
			pushin(Ds[k++], 1);
		}
		if (top > 0) {
			cnt = find(Qs[i].idx, 1);
			//是否比以前的答案要优
			if (cross(stack[cnt], L[Qs[i].pos], mts[Qs[i].idx]) > 0) {
				L[Qs[i].pos] = stack[cnt];
			}
		}
	}
	top = 0;
	for (int i = tot_Q, k = tot_D; i >= 1; i--) {
		while (k >= 1 && Ds[k].x > mts[Qs[i].idx].x) {
			pushin(Ds[k--], -1);
		}
		if (top > 0) {
			cnt = find(Qs[i].idx, -1);
			if (cross(stack[cnt], R[Qs[i].pos], mts[Qs[i].idx]) < 0) {
				R[Qs[i].pos] = stack[cnt];
			}
		}
	}
	if (l < mid) solve(l, mid);
	if (r > mid + 1) solve(mid + 1, r);
}

void getanwser()
{
	double t1, t2;
	int ind;
	for (int i = 1; i <= q; i++) {
		if (query[i].first) {
			ind = query[i].second;
			t1 = atan2(R[i].x - mts[ind].x, R[i].y - mts[ind].y);
			t2 = atan2(mts[ind].x - L[i].x, L[i].y - mts[ind].y);
			printf("%.6lf\n", t1 + t2);
		}
	}
}

int main()
{
	freopen("lhxsb.in", "r", stdin);
	freopen("lhxsb.out", "w", stdout);

	input();
	solve(1, q);
	getanwser();

	fclose(stdin);
	fclose(stdout);
	return 0;
}
